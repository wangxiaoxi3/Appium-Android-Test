...
class ElementActions:
    ...
    def implicitly_wait(self, s):
        return self.driver.implicitly_wait(s)
    ...
    def swip_up(self, count=1, method=None):
        ...
        if count == 1:
            ...
            for x in range(count):
                self.driver.swipe(self.width / 2, self.height * 4 / 5, self.width / 2, self.height * 1 / 5, 2000)
                self.sleep(1)
                try:
                    if method(self):
                        break
    ...
    def is_element_true(self, locator):
        """
        :param
            locator: 定位器
        :return:
    ...
    def screenshot(self, name="takeShot"):
        '''
    ...
    def _find_element_1(self, locator):
        """查找多元素(不会抛异常)
